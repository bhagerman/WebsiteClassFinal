...
game.bop = {
  ...
  draw: function(){
    ...
    atom.context.fillText('Score: ' + this.total, 400, 100);
	atom.context.font = '50px Arial';
	atom.context.fillText('High Score: ' + this.high_score, 450, 270);
  },
  with_key: function(key){
    if (!!(game.activeMole + 1) === true && key === game.holes[game.activeMole].label){
      this.total = this.total+1;
		if (this.total >= this.high_score)
		{
		this.high_score = this.total;
		}
      game.activeMole = -1;
      this.bopped = true;
    }
